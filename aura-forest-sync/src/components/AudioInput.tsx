  ...
  async function startRecording() {
    try {
      ...
      rec.onstop = () => {
        ...
        onChange(blob, `recording-${Date.now()}.webm`);
    ...
    <div className="space-y-5">
      ...
      {mode === "record" ? (
        ...
        <label className="flex flex-col items-center justify-center gap-3 py-10 rounded-2xl border-2 border-dashed border-border bg-secondary/30 cursor-pointer hover:bg-secondary/60 transition-colors">
          ...
          <span className="text-xs text-muted-foreground">MP3, WAV, OGG, M4A, WEBM</span>
          <input
            type="file"
            accept="audio/mpeg,audio/wav,audio/ogg,audio/mp4,audio/x-m4a,audio/webm,audio/*"
            className="hidden"
            onChange={onFile}
          />
        </label>
