...
export function ForestScene3D({
  ...
  const baseTrees = blueprint?.scene_assets?.trees ? 120 : treeCount;
  const adjustedTrees = Math.round(
    Math.min(300, Math.max(40, (baseTrees + forestRangeKm2 * 1.2) * preset.treeMultiplier))
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (blueprint?.scene_assets?.trees && blueprint.scene_assets.trees.length > 0) {
